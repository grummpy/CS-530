...
from fighter.sim.state import MatchState, FighterState, initial_match

def new_match(seed: int = 1, p1_id: str = "graybox_rival", p2_id: str = "graybox_rival", training: int = 0) -> MatchState:
...
def reset_round(match: MatchState) -> None:
    ...
    match.p1, match.p2, match.tick, match.phase, match.events, match.round_ticks = fresh.p1, fresh.p2, fresh.tick, fresh.phase, [], fresh.round_ticks
    ...
    for action, kind, length in ((Action.LIGHT, 1, 12), (Action.MEDIUM, 2, 18), (Action.HEAVY, 3, 25), (Action.SPECIAL, 4, 28), (Action.THROW, 4, 16)):
        if pressed & action:
            if action == Action.SPECIAL:
                if f.special_charge < ARMOR_CHARGE_THRESHOLD: return
                f.special_charge = 0
                if f.fighter_id == "tech_billionaire":
                    f.armor_charge = 0; f.armor_ticks = ARMOR_DURATION_TICKS; events.append("armor_mode_on")
...
def _resolve(attacker: FighterState, defender: FighterState, events: list[str]) -> None:
    if not attacker.attack_ticks or attacker.hit_this_attack: return
    active = attacker.attack_ticks in {attacker.attack_kind + 5, attacker.attack_kind + 6}
    if active and abs(attacker.x - defender.x) < 112:
        damage = 35 * attacker.attack_kind + (3 if attacker.fighter_id == "tech_billionaire" and attacker.armor_ticks else 0)
        if defender.blocking:
            damage = max(1, damage // 3); events.append("block")
        absorbed = damage
...
def tick(match: MatchState, inputs: tuple[InputFrame, InputFrame]) -> None:
    if match.phase != 1: return
    match.events.clear()
    for fighter, frame in zip((match.p1, match.p2), inputs, strict=True):
        fighter.facing = 1 if fighter is match.p1 else -1
        _move(fighter, frame.held); _start_attack(fighter, frame.pressed, match.events)
    _resolve(match.p1, match.p2, match.events); _resolve(match.p2, match.p1, match.events)
    for fighter in (match.p1, match.p2):
        fighter.attack_ticks = max(0, fighter.attack_ticks - 1); fighter.stun_ticks = max(0, fighter.stun_ticks - 1)
        if fighter.armor_ticks:
            fighter.armor_ticks -= 1
            if fighter.armor_ticks == 0: match.events.append("armor_mode_off")
    if not match.training:
        match.round_ticks = max(0, match.round_ticks - 1)
        if match.p1.health == 0 or match.p2.health == 0 or match.round_ticks == 0: match.phase = 2; match.events.append("ko")
    match.tick += 1
