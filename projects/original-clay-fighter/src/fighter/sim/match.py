...
def reset_round(match: MatchState) -> None:
    ...
    match.p1, match.p2, match.tick, match.phase, match.events = fresh.p1, fresh.p2, fresh.tick, fresh.phase, []
...
def _start_attack(f: FighterState, pressed: int, events: list[str]) -> None:
    ...
    for action, kind, length in ((Action.LIGHT, 1, 12), (Action.MEDIUM, 2, 18), (Action.HEAVY, 3, 25), (Action.SPECIAL, 4, 28), (Action.THROW, 4, 16)):
        if pressed & action:
            if action == Action.SPECIAL and f.fighter_id == "tech_billionaire" and f.armor_charge >= ARMOR_CHARGE_THRESHOLD:
                f.armor_charge = 0
...
def tick(match: MatchState, inputs: tuple[InputFrame, InputFrame]) -> None:
    ...
    if not match.training and (match.p1.health == 0 or match.p2.health == 0): match.phase = 2; match.events.append("ko")
    match.tick += 1
